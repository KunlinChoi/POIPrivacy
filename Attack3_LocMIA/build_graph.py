...
if __name__ == '__main__':
    #dst_dir = r'./dataset/NYC/MemInf/lira_4sq/in'
    dst_dir = "./dataset/lira/attack_target"
    # Build POI checkin trajectory graph
    train_df = pd.read_csv(dst_dir+"/graph_train.csv")
            #train_df = pd.read_csv('./dataset/NYC/MemInf/lira_4sq/in/in.csv')
    ...
    for j in range(16):
        for i in range(16):
            dst_dir = "./dataset/NYC/MemInf/lira_4sq/experiment" + str(j) + "/"+str(i)
    # Build POI checkin trajectory graph
            train_df = pd.read_csv(dst_dir+"/in.csv")
            #train_df = pd.read_csv('./dataset/NYC/MemInf/lira_4sq/in/in.csv')
            #train_df = pd.read_csv(os.path.join(dst_dir, 'NYC_train.csv'))
            print('Build global POI checkin graph -----------------------------------')
            G = build_global_POI_checkin_graph(train_df)
...
'''
if __name__ == '__main__':
    dst_dir = r'dataset_4sq/lira/attack_target'

    # Build POI checkin trajectory graph
    train_df = pd.read_csv(os.path.join(dst_dir, 'graph_train.csv'))
    print('Build global POI checkin graph -----------------------------------')
    G = build_global_POI_checkin_graph(train_df)
