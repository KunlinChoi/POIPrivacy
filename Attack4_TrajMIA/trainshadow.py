...
def train(args,train2_path,train3_path):
    ...
    PATH = "./dataset_4sq/lira/dictionary/dict.pt"
    ...
    train_dataset = TrajectoryDatasetTrain2(train_df)
    val_dataset = TrajectoryDatasetVal(val_df)
    print(len(train_dataset),"Adfadf")
    ...
    train_df3 = pd.read_csv(train3_path)
    train_dataset3 = TrajectoryDatasetTrain2(train_df3)
    ...
    for epoch in range(args.epochs):
        ...
        for b_idx, batch in enumerate(train_loader2):
            ...
            src_mask = seq_model.generate_square_subsequent_mask(len(x[0])).to(args.device)
    ...
    def score(X,labels):
        ...
        print('mean acc',np.mean(predictions.argmax(1)==labels[:COUNT]))
    ...
    ####### Load Best ###############
    model_save_dir = os.path.join(args.save_dir, 'checkpoints')
    BPATH = rf"{model_save_dir}/best_epoch.state.pt"
    poi_embed_model.load_state_dict(torch.load(BPATH)["poi_embed_state_dict"])
    node_attn_model.load_state_dict(torch.load(BPATH)["node_attn_state_dict"])
    user_embed_model.load_state_dict(torch.load(BPATH)["user_embed_state_dict"])
    time_embed_model.load_state_dict(torch.load(BPATH)["time_embed_state_dict"])
    cat_embed_model.load_state_dict(torch.load(BPATH)["cat_embed_state_dict"])
    embed_fuse_model1.load_state_dict(torch.load(BPATH)["embed_fuse1_state_dict"])
    embed_fuse_model2.load_state_dict(torch.load(BPATH)["embed_fuse2_state_dict"])
    seq_model.load_state_dict(torch.load(BPATH)["seq_model_state_dict"])
    attn_map = torch.load(BPATH)["node_attn_map"]

    
    for b_idx, batch in enumerate(train_loader3):
        print(b_idx)
        if len(batch) != args.batch:
            src_mask = seq_model.generate_square_subsequent_mask(len(batch)).to(args.device)
        ...
        y_pred_poi, y_pred_time, y_pred_cat = seq_model(x, src_mask)
        last_y = y_pred_poi.to('cpu').tolist()

        X_pred +=last_y

    
    save_p= "./dataset_4sq/lira/prediction/" + train2_path.split("/")[-1][:-3] + "npz"
    print(save_p)
    all_pred = []
    with open(save_p, 'wb') as f:
        for i in range(len(all_traj_id)):
            traj = all_traj_id[i]
            true_y = y[i]
            #pred = X_pred[i]
            #rr1 = [traj] + [true_y] + X_pred
            arr1 = [traj] + [true_y] + X_pred[i]
            all_pred.append(arr1)
    from numpy import savez_compressed
    conf_dict = {}
    in_out_dict = {}
    df_in = pd.read_csv("./dataset_4sq/lira/data/256/0.csv")

    in_traj =list( df_in["trajectory_id"].unique())
    print(len(in_traj))
    for i in range(len(all_pred)):
        traj_id=all_pred[i][0]
        true_y = np.array(all_pred[i][1])
        
        conf =np.array(all_pred[i][2:2+len(true_y)])
        sc = score(conf,true_y)
        conf_dict[traj_id] = sc

        if traj_id in in_traj:
            in_out_dict[traj_id] = 1
        else:
            in_out_dict[traj_id] = 0
    savez_compressed(save_p, dict1=conf_dict,dict2=in_out_dict)
...
if __name__ == '__main__':
    ...
    for j in range(128):
        ...
        train2_p = "./dataset_4sq/lira/data/256/" + str(j) + ".csv"
